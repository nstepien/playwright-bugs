...
test("clicking to restore focus after shift tabbing", async ({
  page,
  browserName,
}) => {
  const logs: string[] = [];
    ...
    case "firefox":
      expect(focusRecords).toEqual(
        [
          ...
          // unlike the previous test, we don't need another click,
          // regardless of whether headless is enabled or not, why?
          // the last state is identical in all three browsers
          {
            documentHasFocus: true,
            iframeDocumentHasFocus: true,
